...
pub async fn op_dns_resolve(
  ...
  {
    let mut name_server_config =
      NameServerConfig::udp_and_tcp(name_server.ip_addr.parse()?);
    for connection in &mut name_server_config.connections {
      connection.port = name_server.port;
    }
    (
      ResolverConfig::from_parts(None, vec![], vec![name_server_config]),
      {
  ...
  lookup
    ...
    .filter_map(|rec| {
      ...
      r.map(|maybe_data| {
        maybe_data.map(|data| DnsRecordWithTtl { data, ttl: rec.ttl })
      })
    })
...
pub fn op_net_get_system_dns_servers() -> Result<Vec<(String, u16)>, NetError> {
  ...
  for ns in config.name_servers() {
    let ip = ns.ip.to_string();
    for connection in &ns.connections {
      let server = (ip.clone(), connection.port);
...
fn format_rdata(
  ...
  move |r: &RData| -> Result<Option<DnsRecordData>, NetError> {
    let record = match ty {
      A => match r {
        RData::A(value) => Some(DnsRecordData::A(value.to_string())),
        _ => None,
      },
      AAAA => match r {
        RData::AAAA(value) => Some(DnsRecordData::Aaaa(value.to_string())),
        _ => None,
      },
      ANAME => match r {
        RData::ANAME(value) => Some(DnsRecordData::Aname(value.to_string())),
        _ => None,
      },
      CAA => match r {
        RData::CAA(caa) => Some(DnsRecordData::Caa {
          critical: caa.issuer_critical,
          tag: caa.tag.to_string(),
          // hickory_proto now handles CAA records encoding within the CAA struct, we can assume that it is safe to unwrap here
          value: str::from_utf8(&caa.value).unwrap().to_string(),
        }),
        _ => None,
      },
      CNAME => match r {
        RData::CNAME(value) => Some(DnsRecordData::Cname(value.to_string())),
        _ => None,
      },
      MX => match r {
        RData::MX(mx) => Some(DnsRecordData::Mx {
      ...
      NAPTR => match r {
        RData::NAPTR(naptr) => Some(DnsRecordData::Naptr {
          order: naptr.order,
      ...
      NS => match r {
        RData::NS(value) => Some(DnsRecordData::Ns(value.to_string())),
        _ => None,
      },
      PTR => match r {
        RData::PTR(value) => Some(DnsRecordData::Ptr(value.to_string())),
        _ => None,
      },
      SOA => match r {
        RData::SOA(soa) => Some(DnsRecordData::Soa {
      ...
      SRV => match r {
        RData::SRV(srv) => Some(DnsRecordData::Srv {
