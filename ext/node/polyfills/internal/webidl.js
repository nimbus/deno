...
// Ported from Node.js v26.10.0 lib/internal/webidl.js.
...
/**
 * Converts a JavaScript value to the IDL USVString type.
 * @see https://webidl.spec.whatwg.org/#es-USVString
 * @param {any} V JavaScript value.
 * @param {ConversionOptions} [options] Conversion options.
 * @returns {string}
 */
converters.USVString = function USVString(V, options = kEmptyObject) {
  return StringPrototypeToWellFormed(converters.DOMString(V, options));
};
...
function createEnumConverter(name, values) {
  // The Deno SafeSet constructor takes an array and wraps it in
  // SafeArrayIterator itself.
...
function createDictionaryConverter(
  ...
  for (let i = 0; i < dictionaries.length; i++) {
    const sortedDictionary = ArrayPrototypeToSorted(
      dictionaries[i],
      compareMembers,
    );
    ...
    for (let j = 0; j < sortedDictionary.length; j++) {
      const member = sortedDictionary[j];
      ...
      setOwnProperty(sortedMembers, sortedMembers.length, {
  ...
  return function (jsDict, options = kEmptyObject) {
    ...
    if (jsDict == null && !hasMissingMemberHandling) return idlDict;

    // Steps 3-4: members retain their per-dictionary order from construction.
  ...
  return function (V, options = kEmptyObject) {
    ...
    while (true) {
      ...
      const idlValue = converter(
        ...
        makeOptions(
          options,
          sequenceElementContext(idlSequence.length, options),
        ),
      );
...
// };
converters.StructuredSerializeOptions = createDictionaryConverter(
  "StructuredSerializeOptions",
  [
    {
      key: "transfer",
      converter: converters["sequence<object>"],
      defaultValue: () => [],
    },
  ],
);

// Node registers this converter in lib/internal/abort_controller.js. Deno's
// AbortSignal comes from deno_web, so register it here.
const { AbortSignalPrototype } = core.loadExtScript(
  "ext:deno_web/03_abort_signal.js",
);
converters.AbortSignal = createInterfaceConverter(
  "AbortSignal",
  (value) => ObjectPrototypeIsPrototypeOf(AbortSignalPrototype, value),
);
