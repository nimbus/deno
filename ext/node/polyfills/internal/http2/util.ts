...
function updateOptionsBuffer(options) {
  ...
  if (typeof options.maxOutstandingSettings === "number") {
    ...
    optionsBuffer[IDX_OPTIONS_MAX_OUTSTANDING_SETTINGS] = MathMax(
      1,
  ...
  if (typeof options.maxSessionMemory === "number") {
    ...
    optionsBuffer[IDX_OPTIONS_MAX_SESSION_MEMORY] = MathMax(
      1,
  ...
  if (typeof options.streamResetRate === "number") {
    ...
    optionsBuffer[IDX_OPTIONS_STREAM_RESET_RATE] = MathMax(
      1,
      options.streamResetRate,
    );
  }
  if (typeof options.streamResetBurst === "number") {
    flags |= 1 << IDX_OPTIONS_STREAM_RESET_BURST;
    optionsBuffer[IDX_OPTIONS_STREAM_RESET_BURST] = MathMax(
      1,
      options.streamResetBurst,
    );
  }
...
function remoteCustomSettingsToBuffer(remoteCustomSettings) {
  ...
  for (let i = 0; i < remoteCustomSettings.length; i++) {
    ...
    if (
      typeof nsetting === "number" && nsetting <= 0xffff &&
      nsetting >= 0
    ) {
      settingsBuffer[IDX_SETTINGS_FLAGS + 1 + 2 * numCustomSettings + 1] =
        nsetting;
...
function prepareRequestHeadersObject(headers, session) {
  ...
  const connect = headersObject[HTTP2_HEADER_METHOD] === HTTP2_METHOD_CONNECT;

  if (!connect || headersObject[HTTP2_HEADER_PROTOCOL] !== undefined) {
    if (getAuthority(headersObject) === undefined) {
    ...
    if (headersObject[HTTP2_HEADER_AUTHORITY] === undefined) {
      throw new ERR_HTTP2_CONNECT_AUTHORITY();
    }
    if (headersObject[HTTP2_HEADER_SCHEME] !== undefined) {
      throw new ERR_HTTP2_CONNECT_SCHEME();
