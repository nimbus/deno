...
const { require } = core.loadExtScript("ext:deno_node/internal/streams/iter/_require.js");
...
function merge(...args) {
  ...
  return {
    ...
    async *[SymbolAsyncIterator]() {
      ...
      // Called when a source's .next() settles. A source is not read again
      // until its queued batch is yielded, bounding memory for fast/infinite
      // producers while keeping one pending read per active source.
      ...
      try {
        ...
        await SafePromiseAll(ArrayPrototypeMap(iterators, async (iterator) => {
          if (iterator.return) {
            try {
