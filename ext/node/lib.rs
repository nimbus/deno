...
deno_core::extension!(deno_node,
  ...
  ops = [
    ...
    ops::vm::op_vm_module_clear_import_meta,
    ops::vm::op_vm_module_get_status,
    ops::vm::op_vm_module_is_graph_async,
    ops::vm::op_vm_module_has_top_level_await,
    ops::vm::op_vm_module_create_cached_data,
    ops::vm::op_vm_module_cached_data_rejected,
    ...
    ops::require::op_require_trace_dynamic_imports,
  ...
  lazy_loaded_esm = [
    ...
    // Keep node:repl after the lazy node:* modules it statically imports
    // (node:fs, node:path, node:console, node:module) so the lazy loader can
    // resolve those dependencies when REPL is pulled in on demand.
    "node:repl" = "repl.ts",
    "node:sea" = "sea_esm.ts",
  ],
  lazy_loaded_js = [
