...
pub enum HeapSnapshotNearHeapLimitPolicy {
  /// Preserve Node/Deno process-lifetime semantics. The installed callback
  /// remains attached to the isolate until it is disposed.
  AllowProcessLifetime,
  /// Reject registration before checking filesystem permissions, allocating
  /// callback state, or attaching a callback to the isolate.
  Deny,
}
...
mod policy_tests {
  ...
  #[test]
  fn deny_policy_fails_before_callback_registration() {
    assert!(matches!(
