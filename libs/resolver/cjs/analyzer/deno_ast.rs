...
mod tests {
  ...
  fn parse_source(source: &str) -> ParsedSource {
    deno_ast::parse_program(ParseParams {
      specifier: url::Url::parse("file:///test/common/fs.js").unwrap(),
      text: std::sync::Arc::<str>::from(source),
  ...
  #[test]
  fn cjs_analysis_keeps_module_exports_object_literal_names() {
    ...
    assert!(analysis.exports.contains(&"nextdir".to_string()));
    assert!(
      analysis
        .exports
